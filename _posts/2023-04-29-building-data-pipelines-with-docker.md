...
To create a Docker network that connects two running containers, follow these steps:

1. Create a Docker network by executing the `docker network create <network_name>` command:

```bash
docker network create python_postgres
```

2. Connect containers to the network using the `docker network connect --alias <container_alias> <network_name> <container_name_or_id>` command:
...
### The App Side
I want to put the `ingest_data.py` script into the `app` folder, here I modify it to read the necessary connection arguments and other configuration parameters from environment variables:
...
NOTE: The environmental variables will be loaded from the `python_connection.env` file in the `/app` sub-directory of the container (`load_dotenv('/app/python_connection.env'` line). The `python_connection.env` file will be mounted to the `/app` sub-directory of the container as a volume during runtime from the `/config` sub-directory of the project. The file content includes:
...
We are almost ready to move to defining the `docker-compose.yml` file. But before that, we have one last step: defining the `postgres_db.env` configuration file in the `/config` sub-directory of the project, which will contain the necessary environment variables for the PostgreSQL database service to set up the database. This file should include configuration details such as the database name, username, password, and any other required settings for the database setup:
...
services:
  database:
    ...
    volumes:
      ...
      - ./config/postgres_db.env:/docker-entrypoint-initdb.d/postgres_db.env:ro
    ports:
      - '5431:5432'
  app:
    build:
...
- The ` - ./database:/var/lib/postgresql/data:rw` instruction of the volumes section maps the ./database directory on the host machine to the /var/lib/postgresql/data directory inside the container. This ensures that the data generated by the database service is persisted in the ./database folder on the host.
- The `- ./config/postgres_db.env:/docker-entrypoint-initdb.d/postgres_db.env:ro` passes the environmental variables (`POSTGRES_USER`, `POSTGRES_PASSWORD`, and `POSTGRES_DB`) to configure the Postgres database by mounting the `postgres_db.env` file as a read-only volume to the path /docker-entrypoint-initdb.d/postgres_db.env inside the container.
...
- The app service is assumed to have its Dockerfile located in the `./app` sub-directory and is built accordingly.
- The `./config/python_connection.env:/app/python_connection.env` instruction ensures that the `python_connection.env` file is available for the script within the container at the `/app` sub-directory, the way I have defined in the updated version of the `ingest_data.py` script.
...
NOTE: Using a volume mapping for the `postgres_db.env` and `python_connection.env` file is a way to provide the necessary sensitive information to the database service and to the app without directly exposing it: these configuration files is stored locally in the `/config` directory outside of the Docker image and are not accessable via `docker inspect` command.

NOTE: The `depends_on` instruction does not guarantee full readiness or availability of the dependent service, rather that the dependency service starts first. Additional mechanisms may be required in a more advanced version of the `compose.yml` file to ensure that the dependent service can successfully connect and interact with the dependency, such as implementing health checks or retries in the application code.
