...
Yes.. it has been a quite long feature request and we are pleased to introduce you the brand new (partial) python binding (kudos [Pablo Hernandez-Cerdan](https://github.com/phcerdan)). At this point, only a few collection of classes/functions are available in the binding (mostly, basicic kernels, some image containers and many topology classes), but everything is set up for further improvements.
